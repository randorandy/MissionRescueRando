...
def assumed_fill(game: Game) -> tuple[bool]:
    ...
    while fill_algorithm.count_items_remaining():
        ...
        if fill_algorithm.count_items_remaining() == 0:
            ...
            #completeGroup = (game.all_locations["Messadon 2 Alpha Missile 1"]["item"], game.all_locations["Messadon 2 Alpha Missile 2"]["item"], game.all_locations["Messadon 2 Alpha Missile 3"]["item"])
            #completable = completeGroup.contains(Items.Morph)
            completable = True
            if completable:
...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"MMR{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/MMR.sfc"
